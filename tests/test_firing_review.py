...
def test_enable_hiring_restores_headcount():
    ...
    # Pin alpha_mean=0.40 (pre-narrative-default-calibration default) to preserve RNG
    # seed sequence for worker theta/wage draws (alpha sampling shifts RNG state).
    params = FirmParams(
        N=100, T=60, tasks_per_worker=5,
        T_review=10.0, firing_threshold=0.0,
        p=0.22, w=1.0, seed=0,
...
def test_hiring_oscillation_bounded():
    ...
    # Pin alpha_mean=0.40 (pre-narrative-default-calibration default) to preserve RNG seed
    # sequence for worker theta/wage draws (alpha sampling shifts RNG state at seed=0).
    params = FirmParams(
        N=100,
        T=30,
        tasks_per_worker=5,
